...
module DiagonalSlidingMovement
  ...
  def up_right_nodes(start_node, distance = 100)
    diagonal_nodes(@board, start_node, 1, 1, distance)
  end

  def up_left_nodes(start_node, distance = 100)
    diagonal_nodes(@board, start_node, -1, 1, distance)
  end

  def down_left_nodes(start_node, distance = 100)
    diagonal_nodes(@board, start_node, -1, -1, distance)
  end

  def down_right_nodes(start_node, distance = 100)
    diagonal_nodes(@board, start_node, 1, -1, distance)
  end
end
